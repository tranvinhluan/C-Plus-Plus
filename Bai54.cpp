/*
	9. Lớp template <=> Lớp nguyên mẫu (template class)
	==> +, chứa các hàm Template
		+, ta có thể truyền tham số vào Lớp
		+, Lớp template viết ra cho nhiều kiểu dữ liệu sử dụng
*/
...
template<class T> Data<T>::Data(T* data, size_t size) {
	this->data = new T[size]; // cấp phát ra mảng kiểu T
	this->size = size; // ghi lại kích thước mảng đó.
